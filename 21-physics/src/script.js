...
debugObject.createCylinder = () => {
  createCylinder(Math.random(), Math.random(), {
    x: (Math.random() - 0.5) * 3,
    y: 3,
    z: (Math.random() - 0.5) * 3,
  });
...
floorBody.mass = 0; //object won't move
...
const createCylinder = (radius, height, position) => {
  ...
  const mesh = new THREE.Mesh(cylinderGeometry, sphereMaterial);
  // mesh.scale.set(radiusTop, radiusBottom, height);
  mesh.castShadow = true;
  mesh.position.copy(position);
  scene.add(mesh);

  const shape = new CANNON.Cylinder(radius, radius, height, 5);
  const body = new CANNON.Body({
    mass: 1,
    position: new CANNON.Vec3(0, 3, 0),
    shape,
